...
namespace GameTutor
{
	...
	void CVSView::KeyFunc(ESContext* esContext, unsigned char key, int x, int y)
	{
		SControllerEvent Event;
		if (!CVSView::GetInstance()) return;
		...
		key &= 0xdf;
		__UINT32 keycode = key;
		// change to Event's keycode
		keycode = keycode << 4;
		
		Event.KeyData.KeyCode = keycode;
		CGame::GetInstance()->SetEvent(Event);		
	}

	void CVSView::KeyUpFunc(ESContext* esContext, unsigned char key)
	{
		SControllerEvent Event;
		if (!CVSView::GetInstance()) return;
